...
@observer
export default class ProductListCNC extends Component {
  ...
  getComponentData = async () => {
    ...
    if(this.props.profile.activeAddress.addressType || isGuestUser) {
      await this.props.products.fetchProductList("shopByCNC", param, extraData);
  ...
  createList = () => {
    ...
    for(let i = 0; i < products.productList.length; i++) {
        if (products.productList[i].cncApplicability) {
            arrr.push(products.productList[i])
        }
    }
    return (
      <FlatList
        ...
        contentContainerStyle={arrr.length === 0 && styles.emptyScreenView}
        onEndReached={() => this.lazyload()}
        onEndReachedThreshold={.7}
        data={(arrr.length > 0) ? arrr.slice() : []}
  ...
  render() {
    ...
    let showlocationFlag = this.props.profile.countryId == 1 ? true : this.props.profile.location_update;
    return (  
      <View style={{flex:1}}>
