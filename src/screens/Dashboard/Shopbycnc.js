...
@observer
export default class Shopbycnc extends Component {
  ...
  handleClick = async (value) => {
    ...
    await this.props.products.fetchProductWidget('shopByCNC', value, this.props.extraData);
  }
  ...
  createPvCart = () => {
    ...
    let { shopByCNC } = products.productWidget || [];
    var shopByPvWidget = [];
    let length = shopByCNC.length > 4 ? 4 : shopByCNC.length
    for (let i = 0; i < length; i++) {
      if (shopByCNC[i].cncApplicability) {

        shopByPvWidget.push(
          <TouchableOpacity
            onPress={() => this.props.navigation.navigate('productDetails', { skuCode: shopByCNC[i].skuCode, locationId: shopByCNC[i].locationId, productId: shopByCNC[i].productId })}
            style={styles.dataContainer}
            key={i.toString()}
          >
            {(get(shopByCNC[i], 'maxQuantity', 0) < 1) ?
              (
            ...
            <View style={styles.itemImageView}>
              ...
              <Banner
                ...
                resizeMode='contain'
                source={(shopByCNC[i].imageUrl) ? { uri: shopByCNC[i].imageUrl } : PRODUCT_PLACEHOLDER}
              />
              {!isGuestUser && (this.props.profile.countryId == 1 || this.props.profile.wishlist_icon == true) &&
                (
                  <TouchableOpacity
                    onPress={() => this.updateWishList(shopByCNC[i], 'add')}
                    style={{ alignSelf: 'flex-end', }}
                  >
                    <Banner
                      styles={styles.favouriteIcon}
                      resizeMode="contain"
                      source={(shopByCNC[i].isFavourite) ? WISHLIST_SELECTED_ICON : WISHLIST_ICON}
                    />
                  </TouchableOpacity>
                )
              }
            </View>
            <Text style={[styles.dataHeading, { marginTop: 18 }]}>{`${Number.parseFloat(get(shopByCNC[i], 'associatedPv', 0)).toFixed(2)} PV`}</Text>
            <Text style={styles.dataHeading}>{`${strings.product.itemCode}: ${shopByCNC[i].skuCode}`}</Text>
            <Text style={styles.dataName} numberOfLines={2}>{capitalizeFirstCharacter(shopByCNC[i].productName)}</Text>
          </TouchableOpacity>
        );
      }
    }
  ...
  render() {
    ...
    for (let i = 0; i < length; i++) {
      if (shopByCNC[i].cncApplicability) {
        arrCNCProducts.push(shopByCNC[i])
      }
    }
    return (
      <View style={styles.mainView}>
        ...
        {(products.isLoading) ? (
          ...
          ((products.productWidget?.shopByCNC && products.productWidget?.shopByCNC.length > 0 && arrCNCProducts.length >= 4) ?
            (
              <View>
                ...
                <TouchableOpacity
                  ...
                  onPress={() => navigation.navigate('productListCNC', { type: 'shopByCNC', title: 'Shop By CNC', param: products.keyValue, extraData: this.props.extraData })}
                >
