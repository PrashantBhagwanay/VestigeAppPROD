/**
 *@description Signup Screen , It should not be called inside logged in route. Used as 
 * self registration. For registration after login seperate downline registration screen will be called.
*/
...
import { connectedToInternet, showToast } from '../../utility/Utility';
...
export default class Signup extends Component {
  ...
  @observable editFosterId: boolean = false;
  ...
  @observable isValidFoster = false;
  ...
  validateReferral = async (value) => {
    ...
    if(res.success && res.data?.isValidDistributor){
      ...
      this.setState({distributorId:fetchReferral, fosterId: fetchReferral, fosterName:res.data.distributorName ,isValidFoster:true})
      AlertClass.showAlert('', 
        `${strings.errorMessage.signUp.referralSuccess} ${strings.errorMessage.signUp.joiningUnder} ${res.data.distributorName}`, 
  ...
  sendOtp = async () => {
    ...
    const resOtp = await this.props.auth.validateMobile({mobileNumber: mobileIsdCode + mobileNumber, countryId: this.countryID});
    if (resOtp.success) {
      this.setModalVisible(true);
    } 
    else {
      this.showToast(resOtp.message);
    }
  }

  showVerifyIcon = () => {
    if(this.countryID != 4 && this.state.mobileNumber.trim().length == this.state.mobileNumberLimit && (this.showVerifyOtp == 2 || this.showVerifyOtp == 3)) return true;
    return false
  }
  ...
  createSignUpForm(): Array<any> {
    ...
    ...(isGHANASelected || isIvoryCoastSelected ? digitalAddressPoDetails : []),
    ];
  ...
  getCoDistributorTitle = () => {
    ...
    else if(this.selectedCountry.toUpperCase() === 'GHANA' || this.selectedCountry.toUpperCase() === 'IVORY COAST') {
...
handleTextInput = async (key, text) => {
  ...
  switch (key) {
    case SIGNUP_KEY_ENUM.DISTRIBUTOR_ID: {
      this.setState({ uplineName: '' });
      this.editFosterId=false;
      ...
      if (text.length === 8) {
        ...
        const res = await this.props.profile.validateFosterWithoutLogin(text, this.state.distributorId);
...
/**
 * @description this is used to return location id as per requirement  for age validation.
 */
...
isValidate = () => {
  ...
  const isGHANASelected = this.state.selectedMobileCountryCode.toUpperCase() === 'GHANA';
  const isIvoryCoastSelected = this.state.selectedMobileCountryCode.toUpperCase() == 'CI';
  console.log("isGHANASelected==>"+isGHANASelected)
  ...
  else if((isGHANASelected || isIvoryCoastSelected) && !this.state.digitalAddressGhana.trim()) {
  ...
  if(this.selectedDistributor === 'Distributor ID') {
    return strings.errorMessage.signUp.invalidDistributor
  }
}

/**
 * @description this will return DOB validation message based on country or location
 */
...
submitData = async() => {
  ...
  const user = {
    ...
    fosterId: this.state.fosterId ? this.state.fosterId : this.referral,
  ...
  // console.log(user)
  const jsonResponse = await this.props.auth.signup(user);
  let mess = strings.errorMessage.signUp.registrationFailed;
  if (jsonResponse && jsonResponse.message ) {
    mess = jsonResponse.message
  }
  (jsonResponse && !jsonResponse.message && Object.keys(jsonResponse).length > 0) ?
    (
      setTimeout(() => {
        Alert.alert(
  ...
  if(selectedPickerIndex !== 0){
    switch(selectedPickerKey) {
      ...
      case PICKER_ENUM.COUNTRY_PICKER : {
        ...
        await location.countryListData.filter(async (country)=>{
...
async verifySignupOtp(otp) {
  ...
  const mobileWithISD = mobileIsdCode + mobileNumber;

  const resOtpVerify = await this.props.auth.signupOtpVerification(mobileWithISD, otp)
  if(resOtpVerify.success){
    return true
  }
  else{
    return false
  ...
  return(
    <View style={styles.container}>
      ...
      >
        ...
        >    
          <Loader loading={Platform.OS === 'ios' && this.props.auth.isLoading} />  
          ...
          <View style={{flexDirection: 'row', marginTop: 30, marginBottom: 15, alignItems: 'center',}}>
            <OTPInput
              ...
              verifySignupOtp={(code) => this.verifySignupOtp(code)}
              autoFocus
