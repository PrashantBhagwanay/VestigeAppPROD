...
export default class OTPInput extends Component {
  ...
  componentDidMount() {
    ...
    //   console.log('Invalid props: compareWith length is not equal to codeLength');
    // }
  ...
  // setOtp(otp) {
  //   const {compareWithCode,onFulfill}=this.props;
  //   this.setState({
  //     codeArr: otp.split(''),
  //     currentIndex: 3
  //   },() => {
  //     setTimeout(()=>{
  //       if(otp === compareWithCode){
  //         onFulfill(true, otp)
  //       }
  //     },200)
  //     this._blur(0); 
  ...
  async _onInputCode(character, index) {
    ...
    const { codeLength, onFulfill, compareWithCode, ignoreCase, verifySignupOtp } = this.props;
    let newCodeArr = _.clone(this.state.codeArr);
    ...
    if (index == codeLength - 1) {
      ...
      // if (compareWithCode) {
      const isMatching = await verifySignupOtp(code);
