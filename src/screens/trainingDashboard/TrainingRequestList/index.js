...
import TrainingTypeFilter from './component/TrainingTypeFilter';
...
const TRAINING_TYPE_LIST = [
  TRAINING_STATUS_ENUM.ALL,
  TRAINING_STATUS_ENUM.PENDING,
  TRAINING_STATUS_ENUM.APPROVED,
  TRAINING_STATUS_ENUM.REJECTED,
...
class TrainingRequestList extends Component {
  ...
  constructor(props) {
    ...
    this.props = props;
    this.state = {
      selectedFilter: TRAINING_STATUS_ENUM.ALL,
      trainingList: [],
      rejectReasons: [],
  ...
  getFilteredTrainingList = (trainingList, selectedFilter) => {
    if (selectedFilter === TRAINING_STATUS_ENUM.ALL) {
      return trainingList;
    }
    if(selectedFilter === TRAINING_STATUS_ENUM.REJECTED) {
      const filteredData = trainingList?.filter(item => this.searchWord(item?.Status?.toUpperCase(),'REJECTED'))
      return filteredData;
    }
    console.log('selected Filter',selectedFilter)
    const filteredData = trainingList?.filter(
      item => item?.Status?.toUpperCase() === selectedFilter?.toUpperCase(),
    );
    return filteredData;
  };
  ...
  render() {
    const { trainingList, selectedFilter } = this.state;
    ...
    return (
      <View style={{flex: 1}}>
        ...
        />
        <TrainingTypeFilter
          filterList={TRAINING_TYPE_LIST}
          selectedFilter={selectedFilter}
          handleFilterCallback={this.handleFilterCallback}
          setFilter={this.handleFilterCallback}
        />
        <View style={styles.main}>
          <FlatList
            data={this.getFilteredTrainingList(trainingList, selectedFilter)}
