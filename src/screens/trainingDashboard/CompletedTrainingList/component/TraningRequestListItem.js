...
const TrainingRequestListItem = props => {
  ...
  const isAnyButtonEnabled = isActiveReimburseId=='1'?true:false;
  const RemarksMessage =Remarks;
  const RejectedReimburseReason=reason;
    ...
    <View style={styles.listItemContainer}>
      ...
      {isAnyButtonEnabled && (
        <View style={[styles.footerContainer, { height: isEditEnabled ? 40 : 50 }]}>
          ...
          <CustomButton
              ...
              buttonStyle={{ width: Dimensions.get('window').width /2 - 25, height: 40 }}
              // buttonTextStyle={{ padding: 3 }}
              onPress={() => props.navigation.navigate('claimentInfo', { trainingId: TrainingRequestID })}
            />
          </View>)}
        </View>
      )}
      {RemarksMessage&&(
          <View style={{margin:10,}}>
              <Text style={[
                Reimburse_status=='1'? styles.remarkMessage 
              : (Reimburse_status=='3'|| Reimburse_status=='2')?styles.remarkMessageSuccess
              :Reimburse_status=='4'?styles.remarkMessageError:styles.remarkMessage ,
               styles.detailLabelRightSide]}>
              {RemarksMessage}
            </Text>
            </View>
      ...
      {(RejectedReimburseReason!=""&&Reimburse_status=='4') && (
         <View style={{marginTop:1,marginLeft:10}}>
              <Text style={[
                 styles.remarkMessageError:styles.remarkMessage ,
               styles.detailLabelRightSide]}>
              {`Reject Reason : ${RejectedReimburseReason}`}
            </Text>
            </View>
      )}
